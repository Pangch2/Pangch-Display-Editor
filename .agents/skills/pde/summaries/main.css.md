...
## Purpose
Defines renderer UI styling, including independent left/right panel docks, project and selected-object property forms, project tab navigation/dropdown, canvas context/submenus, panel drag/drop previews, adjustable divider, scene tree, loading overlay, and FPS counter.
...
- Each dock has a visible 7 x 48 px resize handle centered completely outside its edge, keeping panel scrollbars unobstructed; dragging within 10% of the screen edge shows a 10 px blue minimize strip, while dragging a minimized handle inward by 140 px shows a 280 px restore preview; a minimized dock hides everything except its handle, and a shared dock shows its 12 px section divider.
- Project Details inputs keep their neutral border on hover, fill their column, and remain single-line.
- The global-brightness project row lays out its native checkbox and sky/block selects in one compact grid; the selects match object-property control typography and padding.
- Object properties use border-box sizing, compact XYZ rows, Blockbench scale direction arrows, a fixed-size symbol-font 4x4/comma-separated one-line matrix toggle whose compact input sits flush against its wrapper and shares the standard 3 px input corner radius, a muted non-editable final row, single-line NBT inputs, dark native number spinners, full-width native metadata selects, and sky-blue before/after markers while reordering metadata rows or the transform/matrix/NBT/metadata sections.
- Object-property name headings use full-width text inputs; scene rows give their compact dark F2 rename input the available label width and temporarily hide extra metadata.
- The individual-selection property list uses a relative root, height spacer, and absolutely positioned viewport content; mounted sections disable content visibility because TypeScript bounds their DOM count and measures their real heights, while native scroll anchoring is disabled for explicit anchor compensation.
