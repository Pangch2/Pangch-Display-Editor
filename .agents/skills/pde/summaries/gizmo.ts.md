...
- TransformControls caches merged selected ranges at drag start; `updateGizmo()` changes one shared cumulative drag uniform per frame while applying reflected incremental deltas to linked mirror partners and incremental deltas to overlays and drag-time UI events.
- Drag end commits selected CPU and outline matrices, tightens the aggregate selection box once, then immediately resets the GPU preview; linked mirror partners have already received their CPU matrices incrementally during the drag.
...
- Mirror-modeling duplication creates one duplicate, restores the pre-duplication source selection, reflects each group transform and origin before its child object matrices, pairs custom pivots, then finishes asynchronous block/player-head replacement and object pairing. Group children use center-mode object reflection like direct group flips so baked item-display geometry stays within group bounds. Blocks/group transforms reflect across X=-0.5, while centered item displays, default/custom group origins, and player-head logical transforms reflect across X=0.
- In mirror-modeling mode, delete expands the direct selection to linked partners, while group creation builds and pairs one group per side before restoring the source group selection.
...
- Direct group flips use the live selection-bounds center so reflection stays in place, apply the group reflection before child object matrices so the pre-flip default origin is preserved, and update selected root and descendant group transforms. Non-group flips continue to honor the active pivot mode.
...
- Single-group gizmo placement reads the stored group custom pivot directly as a world position.
