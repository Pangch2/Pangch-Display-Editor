...
## Purpose
Implements scene-panel data lookup and mutation helpers that operate on `loadedObjectGroup.userData`. It resolves groups and objects in scene order, maps visual drop hints to insertion points, and applies moves back into the scene model.
...
- `ensureSceneOrderSeeded(ud): SceneOrderEntry[]` -- initializes root ordering from virtual row metadata when needed.
- `getParentGroupIdFromElement(el): string | null` -- returns `data-parent-group-id` for virtual rows, with legacy DOM ancestry fallback.
...
- `./scene-panel-state` -- virtual row metadata for root-order seeding.
...
- `scene-panel-render.ts` -- uses lookup helpers while building virtual rows.
