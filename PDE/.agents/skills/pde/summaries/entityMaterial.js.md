...
### Functions / Methods
- `createEntityMaterial(diffuseTex, tintHex = 0xffffff, useInstancedUv = false, useInstancedUvTransform = false)` -- returns `{ material, blockLightLevel, skyLightLevel }` for entity rendering and light-level control.
...
- Switches UV lookup to `instancedUvTransform` vec4 scale/offset when requested, otherwise to `instancedUvOffset` when `useInstancedUv` is true.
...
- `useInstancedUvTransform` expects geometry to provide `instancedUvTransform` as `[scaleU, scaleV, offsetU, offsetV]`.
