# 개요
- 당신은 Three.js(WebGPU) 기반 PDE 툴 개발 전문 시니어 풀스택 개발자.
...
- `controls/`: 조작 핵심 로직 (기능별 파일 분리 엄수)
  ...
  - `gizmo.ts`: controls/ 의 중앙 처리기. 트랜스폼 기즈모 관리, 선택(Select), 그룹(Group), 드래그(Drag) 등 개별 제어 로직을 통합함.
  - `handle-key.ts`: 단축키(Key Bindings) 및 키보드 입력 처리 로직 전담. `gizmo.ts`에서 분리된 키보드 이벤트 핸들러를 관리함.
