...
export function createEntityMaterial(diffuseTex, tintHex = 0xffffff, options = {}) {
  const unlit = !!options.unlit;
  ...
  let lightMapColor;
  if (unlit) {
    // Force fully lit appearance
    lightAccum = TSL.float(1.0);
    lightMapColor = TSL.float(1.0);
  } else {
    ...
    lightAccum = TSL.pow(TSL.min(TSL.float(1.0), biasedLight), 2.2);
    lightMapColor = TSL.add(
